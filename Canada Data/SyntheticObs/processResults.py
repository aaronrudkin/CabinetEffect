...
writer.writerow(['ID', 'Name.x', 'Incumbent', 'Date','Elected','VotePct', 'CabinetNow','CabinetImportant','CabinetPM','TermsServed','Party', 'Gender', 'Province', 'PartyInGovt', 'LastVotePct', "PastLawJob", 'Stub'])
...
for row in reader:
	...
	id, name, _, date, termsServed, lastVote, gender, province, party, pastlawjob, _ = row
	...
	pastlawjob = int(pastlawjob)
	party = str(party)
	date = str(date)
	lastVote = float(lastVote)
	servedNow = 0
	important = 0
	pm = 0
	incumbent = 1
	...
	writer.writerow([id, name, 1, currElecDate, elected, votePct, servedNow, important, pm, termsServed, party, gender, province, partyInGovt, lastVote, pastlawjob, 1])
