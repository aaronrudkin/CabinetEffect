...
provMapForSpeaking = {"ON": "Ontario", "AB": "Alberta", "BC": "British Columbia", "MB": "Manitoba", "QC": "Quebec", "PE": "Prince Edward Island", "NB": "New Brunswick", "NS": "Nova Scotia","SK": "Saskatchewan", "YT": "Yukon","NU": "Nunavut", "NT": "Northwest Territories","NL": "Newfoundland and Labrador"}
...
for bDate in bDates:
	...
	if len(dob)==0:
		...
		if len(dob)==7:
			...
			print len(dob), "error bad line"
			sys.exit()
		actualBDs.append(dob)
...
def loadGender(filename):
	gender = csv.reader(open(filename, "r"))
	gSet = []
	i=0
	for line in gender:
		i=i+1
		if i==1:
			...
			name, party, riding, province, elected, electionnum = [x.strip().lower() for x in line]
			name = unidecode.unidecode(name.decode("utf-8").lower())
			riding = unidecode.unidecode(riding.decode("utf-8").lower())
			line_g = {"name": name, "party": party, "riding": riding, "province": province, "elected": int(elected), "electionnum": int(electionnum)}
			gSet.append(line_g)
...
gender_data = loadGender("gender_out.csv")
print len(gender_data), " female candidates."
print gender_data[10:12]

def loadSpeaking(filename):
	speaking = csv.reader(open(filename, "r"))
	sSet = []
	i = 0
	for line in speaking:
		i=i+1
		if i==1:
			continue
		else:
			junk, pid, parliament_num, name, dob, gender, start_date, end_date, riding_name, province, party_name, speaking_words, num_bills_passed, num_bills_failed, election_num = [x.strip().lower() for x in line]
			name = unidecode.unidecode(name.decode("utf-8").lower())
			names = name.rsplit(" ",1)
			name = names[1] + ", "+names[0]
			riding_name = unidecode.unidecode(riding_name.decode("utf-8").lower())
			line_s = {"name": name, "riding_name": riding_name, "province": provMapForSpeaking[province.upper()].lower(), "party_name": party_name, "speaking_words": int(speaking_words), "num_bills_passed": int(num_bills_passed), "num_bills_failed": int(num_bills_failed), "election_num": int(election_num)}
			sSet.append(line_s)

	return sSet

print "Loading speaking data"
speaking_data = loadSpeaking("merged_speaking.csv")
print len(speaking_data), " speakers."
...
writer.writerow(['ID', 'Name', 'Age', 'Gender', 'ParlSpeaking', 'Incumbent', 'TermsServed', 'CabinetNow', 'CabinetEver', 'CabinetImportant', 'CabinetPM', 'Date', 'Party', 'Riding', 'Province','Votes', 'VotePct', 'Elected', 'LastVotes', 'LastVotePct', 'Job', 'PastPoliJob', 'PastLawJob', 'PastBusinessJob', "NumSpokenLines", "NumBillsFailed", "NumBillsPassed"])
...
for line in lines:
	...
	if electionnum==1:
		...
		if len(firstPassSubset):
			...
			if len(exactMatch)==1:
				...
				polEver = exactMatch[0]["polEver"]
				lawEver = exactMatch[0]["lawEver"]
				businessEver = exactMatch[0]["businessEver"]
				...
				if not len(exactFamilyNameMatch):
					...
					if score>=45 or (score==45 and len(firstname)==0) or (score>=30 and preID==0 and exactFamilyNameMatch[0]["preID"]==0 and ("full name match" in flags or (("party match" in flags or "party nickname match" in flags) and ("name match" in flags or "1-to-2 name match" in flags)))):
						...
						polEver = exactFamilyNameMatch[0]["polEver"]
						lawEver = exactFamilyNameMatch[0]["lawEver"]
						businessEver = exactFamilyNameMatch[0]["businessEver"]
					...
					if len(plausibleResults)==0:
						...
						polEver = plausibleResults[0]["polEver"]
						lawEver = plausibleResults[0]["lawEver"]
						businessEver = plausibleResults[0]["businessEver"]
	...
	# Gender labels.
	if electionnum>=14:
		prov_elected_cand_this_election = [x for x in gender_data if x["electionnum"]==electionnum and x["province"]==province.lower() and x["elected"]==elected]
		if len(prov_elected_cand_this_election):
			exact_match = [x for x in prov_elected_cand_this_election if x["name"].lower() == name.lower()]
			if len(exact_match) == 1:
				if levenshtein(exact_match[0]["riding"].lower(), unidecode.unidecode(riding.decode("utf-8").lower())) >= 0.90:
					win_gmatch=win_gmatch+1
					gender=1
					#print exact_match[0]
					#print party, riding, elected, name, lastname, firstname
			elif len(exact_match) > 1:
				triple_match = [x for x in prov_elected_cand_this_election if x["name"].lower() == name.lower() and x["riding"].lower() == riding.lower()]
				if not len(triple_match) or len(triple_match)>1:
					print "Severe error"
					print exact_match
					print party, riding, elected, name, lastname, firstname
					raw_input("NOOOO")
					...
					win_gmatch=win_gmatch+1
					gender=1
					print "Multi match, but only one is perfect."
			else:
				for c in prov_elected_cand_this_election:
					r_score = levenshtein(riding.replace("-"," ").replace("  "," ").lower(), c["riding"].replace("-"," ").replace("  "," ").lower()) 
					n_score = levenshtein(name.replace("-"," ").replace("  "," ").lower(), c["name"].replace("-"," ").replace("  "," ").lower())
					if r_score >= 0.9 and n_score >= 0.9:
						win_gmatch=win_gmatch+1
						gender=1
						break

	# Speaking labels
	if electionnum>=36 and electionnum % 1 == 0 and incumbent==1:
		last_election_speaking = [x for x in speaking_data if x["election_num"]==electionnum and x["province"].lower()==province.lower()]

		exact_match = [x for x in last_election_speaking if x["party_name"].lower()==party.lower() and x["name"].replace("-"," ").replace("  "," ").lower() == name.lower() and x["riding_name"].replace("-"," ").replace("  "," ").lower() == riding.replace("-", " ").replace("  ", " ").lower()]
		if len(exact_match)>1:
			print exact_match
			print name, riding, party, province, electionnum
			raw_input("Serious error: Multiple exact matches")
		elif len(exact_match)==1:
			num_spoken_lines = exact_match[0]["speaking_words"]
			num_bills_failed = exact_match[0]["num_bills_failed"]
			num_bills_passed = exact_match[0]["num_bills_passed"]
			win_smatch = win_smatch + 1
		else:
			gm = 0
			best_r = 0
			best_n = 0
			best_item = {}
			for c in last_election_speaking:
				r_score = levenshtein(riding.replace("-"," ").replace("  "," ").lower(), c["riding_name"].replace("-"," ").replace("  "," ").lower())
				n_score = levenshtein(name.replace("-"," ").replace("  "," ").lower(), c["name"].replace("-"," ").replace("  "," ").lower())

				if (r_score > 0.8 and n_score > 0.8) or (n_score >= 1.0) or (r_score >= 0.95 and n_score >= 0.7):
					num_spoken_lines = c["speaking_words"]
					num_bills_failed = c["num_bills_failed"]
					num_bills_passed = c["num_bills_passed"]
					win_smatch = win_smatch + 1
					gm=1
					break
				else:
					if r_score+n_score > best_r+best_n:
						best_r = r_score
						best_n = n_score
						best_item = c

			if not gm:
				if name.lower().startswith(best_item["name"].lower()) or best_item["name"].lower().startswith(name.lower()) or best_r+best_n>1.3:
					num_spoken_lines = best_item["speaking_words"]
					num_bills_failed = best_item["num_bills_failed"]
					num_bills_passed = best_item["num_bills_passed"]
					win_smatch = win_smatch + 1
					gm=1
					#print best_r, best_n, best_item["name"], name, best_item["riding_name"], riding
					#raw_input("Serious error: No match at all, but good followup match")
				else:
					print best_item
					revised_best_name = " ".join(sorted(best_item["name"].replace(",","").replace(".","").split(" ")))
					revised_active_name = " ".join(sorted(name.replace(",","").replace(".","").split(" ")))
					if levenshtein(revised_best_name, revised_active_name) > 0.9 or "st. denis" in name.lower() or "van kesteren" in name.lower():
						num_spoken_lines = best_item["speaking_words"]
						num_bills_failed = best_item["num_bills_failed"]
						num_bills_passed = best_item["num_bills_passed"]
						win_smatch = win_smatch + 1
						gm=1
						print revised_best_name, "///", revised_active_name
						#raw_input("Serious error: No match at all, but good followup match")
					else:
						print "Bad match"
						print revised_best_name, "///", revised_active_name
						print name, riding, party
						print best_r, best_n, best_item				
						#raw_input("Serious error: No match at all.")

		
		

	# Job labels
	if incumbent==1 or job.lower().strip() in polJobs:
		polEver = 1
		win_jmatch = win_jmatch + 1
	elif job.lower().strip() in lawJobs:
		lawEver = 1
		win_jmatch = win_jmatch + 1
	elif job.lower().strip() in businessJobs:
		businessEver = 1
		win_jmatch = win_jmatch + 1
	...
	currentRow = {"province": province, 
			...
			"important": important, "job": job, "matchedNext": -1, "rowIndex": len(rowSet),
			"gender": gender, "num_spoken_lines": num_spoken_lines, "num_bills_passed": num_bills_passed, "num_bills_failed": num_bills_failed,
			"polEver": polEver, "lawEver": lawEver, "businessEver": businessEver}


	writer.writerow([idNumber, name, ageAtElection, gender, num_spoken_lines, incumbent, incumbentTerms, servedNow, servedEver, important, pm, date, party, riding, province, votes, percent, elected, lastVotes, lastVotePct, job, polEver, lawEver, businessEver, num_spoken_lines, num_bills_failed, num_bills_passed])
...
print "female candidates total: ", len(gender_data), " -- and identified female candidates: ", win_gmatch
print "number of candidates with some qualification in job: ", win_jmatch
print "number of speakers: ", len(speaking_data), " -- and identified: ", win_smatch
